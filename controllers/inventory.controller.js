...
class InventoryController {
    ...
    async create(req, res) {
        try {
            ...
            logger.info(`Inventory item created: ${item.itemCode} by ${req.user.username}`);
    ...
    async update(req, res) {
        try {
            ...
            logger.info(`Inventory item updated: ${item.itemCode} by ${req.user.username}`);
    ...
    async delete(req, res) {
        try {
            ...
            // Emit WebSocket event
            if (req.app.get('socketio')) {
                req.app.get('socketio').emit('inventory:deleted', { id: req.params.id });
            }
            
            logger.info(`Inventory item deleted: ${itemCode} by ${req.user.username}`);
    ...
    async adjustStock(req, res) {
        try {
            ...
            logger.info(`Stock adjusted: ${item.itemCode} (${adjustment}) by ${req.user.username}`);
    ...
    async getStatistics(req, res) {
        try {
            ...
            const outOfStockItems = await Inventory.count({ where: { currentStock: 0 } });
            
            const byCategory = await Inventory.findAll({
                attributes: [
                    'category',
                    [require('sequelize').fn('COUNT', '*'), 'count'],
                    [require('sequelize').fn('SUM', require('sequelize').col('totalValue')), 'value']
                ],
                group: ['category']
            });
