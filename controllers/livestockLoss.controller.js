...
const { Livestock, LivestockLoss, User } = require('../models');
const logger = require('../config/logger');

class LivestockLossController {
    async getAll(req, res) {
        try {
            const { livestockId, reasonForLoss, status, page = 1, limit = 20 } = req.query;
            const where = {};
            ...
            const { rows: losses, count } = await LivestockLoss.findAndCountAll({
                ...
                include: [{ model: Livestock, as: 'livestock', attributes: ['id', 'animalCode', 'name'] }]
            });
            ...
            res.status(500).json({ error: 'Failed to fetch losses', details: error.message });
        }
    }
    ...
    async getById(req, res) {
        try {
            ...
            if (!loss) return res.status(404).json({ error: 'Loss record not found' });
            res.json({ loss });
        } catch (error) {
            logger.error('Get loss error:', error);
            res.status(500).json({ error: 'Failed to fetch loss', details: error.message });
        }
    }
    ...
    async create(req, res) {
        try {
            ...
            res.status(500).json({ error: 'Failed to create loss record', details: error.message });
        }
    }
    ...
    async update(req, res) {
        try {
            ...
            res.status(500).json({ error: 'Failed to update loss record', details: error.message });
        }
    }
    ...
    async delete(req, res) {
        try {
            ...
            res.status(500).json({ error: 'Failed to delete loss record', details: error.message });
        }
    }
    ...
    async approveLoss(req, res) {
        try {
            ...
            res.status(500).json({ error: 'Failed to approve loss', details: error.message });
        }
    }
    ...
    async rejectLoss(req, res) {
        try {
            ...
            res.status(500).json({ error: 'Failed to reject loss', details: error.message });
        }
    }
}
