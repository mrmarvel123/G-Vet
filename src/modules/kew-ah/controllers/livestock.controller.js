...
const { Livestock, User } = require("../../../database/models");
...
class LivestockController {
  ...
  async create(req, res) {
    try {
      ...
      logger.info(
        `Livestock created: ${animal.animalCode} by ${req.user.username}`,
      );
  ...
  async update(req, res) {
    try {
      ...
      logger.info(
        `Livestock updated: ${animal.animalCode} by ${req.user.username}`,
      );
  ...
  async delete(req, res) {
    try {
      ...
      if (req.app.get("socketio")) {
        req.app
          .get("socketio")
          .emit("livestock:deleted", { id: req.params.id });
      }
  ...
  async getStatistics(req, res) {
    try {
      ...
      const healthyLivestock = await Livestock.count({
      ...
      const deceasedLivestock = await Livestock.count({
        where: { healthStatus: "Deceased" },
      });
